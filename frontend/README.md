...
```bash
cd ../backend
USE_LOCAL_MOCK_AUTH=true func start --port 7071
```
...
In `e2e` mode the app always uses local `/api` requests through the Vite proxy.

### Mock identity override

The frontend reads mock auth state from localStorage key `sentinel:e2e-auth`.
